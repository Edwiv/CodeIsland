...
- Restore smooth, centered Island expansion while retaining compact screenshot bounds by separating the NSPanel envelope switch from the SwiftUI spring animation
- Stop accumulating dormant Observation registrations and skip redundant window-frame submissions, preventing expansion jank from worsening over long uptimes
...
- 在保留截图所需紧凑窗口边界的同时恢复居中、丝滑的 Island 展开动效；NSPanel 承载区切换不再与 SwiftUI 弹簧动画互相驱动
- 修复 Observation 监听器随运行时间不断叠加的问题，并跳过重复窗口 frame 提交，避免展开卡顿随常驻时间持续恶化
